      ...
      React.createElement('div', {
        ...
        React.createElement('div', {
          ...
          React.createElement('div', {
            ...
            React.createElement('span', {
              key: 'cost',
              className: 'text-sm font-bold text-green-600 ml-2'
            }, `$${piece.cost}`)
          ])
        ]),
      ...
      React.createElement('div', {
        ...
        React.createElement('div', {
          ...
          React.createElement('div', { key: 'details', className: 'min-w-0 flex-1' }, [
            ...
            }, `${group.pieceCount} pieces • $${group.totalCost}`),
            React.createElement('div', {
              key: 'dimensions',
              className: 'text-xs text-gray-400'
            }, group.boundingBox ? 
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        React.createElement('div', {
          key: 'cost-stat',
          className: 'bg-white rounded-lg p-3 text-center border border-green-200 hover:shadow-sm transition-shadow'
        }, [
          React.createElement('div', { key: 'number', className: 'text-2xl font-bold text-green-600' }, `$${stats.totalCost}`),
          React.createElement('div', { key: 'label', className: 'text-sm text-gray-600' }, 'Total Cost'),
          React.createElement('div', { key: 'avg', className: 'text-xs text-gray-500 mt-1' }, `$${Math.round(stats.totalCost / Math.max(1, pieces.length))} avg`)
        ]),
        React.createElement('div', {
          key: 'openings-stat',
      ...
      selectedForGrouping.length > 0 && React.createElement('div', {
        ...
        React.createElement('div', {
          ...
          React.createElement('div', {
            ...
            }, `$${selectedForGrouping.reduce((sum, id) => {
              const piece = pieces.find(p => p.id === id);
              return sum + (piece?.cost || 0);
            }, 0)}`)
          ]),
          React.createElement('button', {
            key: 'clear-selection',
